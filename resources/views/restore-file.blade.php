<x-app-layout>
    <x-slot name="header">Restore File OJS</x-slot>

    <div class="space-y-6 max-w-4xl">

        <div>
            <h2 class="text-xl font-semibold text-gray-900 mb-2">Restore File OJS</h2>
            <p class="text-sm text-gray-500">Kembalikan file jurnal OJS (cover, submission, galley) dari file ZIP backup.</p>
        </div>
        ...
        <div class="bg-amber-50 border border-amber-200 rounded-lg p-4">
            <div class="flex gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-amber-800">File yang ada di folder OJS dengan nama yang sama akan ditimpa. Pastikan menggunakan file ZIP dari fitur <strong>Backup File</strong> sistem ini.</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6">
            <h3 class="font-semibold text-gray-900 mb-3">Tujuan Restore</h3>
            <div class="bg-gray-50 p-4 rounded-lg flex items-center gap-3">
                <svg class="w-5 h-5 text-gray-400 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>
                <div>
                    <p class="font-medium text-gray-900 text-sm font-mono">{{ $ojsPath }}</p>
                    <p class="text-xs text-gray-500 mt-0.5">
                        @if($exists) <span class="text-green-600 font-medium">✓ Folder ditemukan</span>
                        @else <span class="text-red-600 font-medium">✗ Folder tidak ditemukan — akan dibuat otomatis</span>
                        @endif
                    </p>
                </div>
            </div>
        ...
        {{-- Upload ZIP --}}
        <div class="bg-white rounded-lg border border-gray-200 p-6">
            <h3 class="font-semibold text-gray-900 mb-4">Upload File Backup ZIP</h3>
            <div id="drop-zone"
                ...
                <p class="text-gray-700 mb-2">Drag & drop file backup ZIP atau <span class="text-blue-600 font-medium">pilih file</span></p>
                <p class="text-sm text-gray-500">Format: backup_ojs_files_*.zip (Maks 512MB)</p>
        ...
        <div id="restore-log-section" class="hidden bg-white rounded-lg border border-gray-200 p-6">
            <h3 class="font-semibold text-gray-900 mb-4">Status Restore File</h3>
            <div id="restore-log" class="bg-gray-900 rounded-lg p-4 font-mono text-sm text-gray-300 space-y-1 min-h-24 max-h-60 overflow-y-auto"></div>
            ...
            <div id="restore-success" class="hidden mt-4 bg-green-50 border border-green-200 rounded-lg p-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="font-semibold text-green-900">Restore file selesai!</p>
                        <p id="restore-detail" class="text-sm text-green-700 mt-0.5"></p>
                    </div>
                </div>
            </div>
    ...
    function handleDrop(event) {
        ...
        if (file) { selectedFile = file; updateFileUI(file.name); }
    }
    ...
    function startRestore() {
        ...
        const logSec    = document.getElementById('restore-log-section');
        const logDiv    = document.getElementById('restore-log');
        const successDiv = document.getElementById('restore-success');
        const errDiv    = document.getElementById('restore-error');
        ...
        const steps = [
            'Memvalidasi file ZIP...',
            'Membuka arsip...',
            'Mengembalikan file cover jurnal...',
            'Mengembalikan file submission...',
            'Mengembalikan file galley (PDF/DOCX)...',
            'Memverifikasi integritas file...',
        ];
        let i = 0;
        const fakeLogs = setInterval(() => {
            if (i < steps.length) { appendLog(steps[i]); i++; }
            else clearInterval(fakeLogs);
        }, 500);
        ...
        fetch('{{ route('restore-file.run') }}', { method: 'POST', body: formData })
        .then(r => r.json())
        .then(data => {
            ...
            if (data.success) {
                ...
                document.getElementById('restore-detail').textContent =
                    data.extracted + ' file berhasil dikembalikan' +
                    (data.skipped > 0 ? ', ' + data.skipped + ' file dilewati.' : '.');
            } else {
        ...
        .catch(e => {
            clearInterval(fakeLogs);
            appendLog('Koneksi error: ' + e.message, 'error');
            errDiv.classList.remove('hidden');
            document.getElementById('restore-error-msg').textContent = e.message;
            btn.disabled = false;
            btn.classList.remove('opacity-60');
        });
    }
