      ...
      <div class="max-w-md w-full">
        <div class="bg-white rounded-2xl shadow-xl p-8">
          ...
          <form method="POST" action="{{ route('login') }}" class="space-y-6">
            ...
            <!-- Identifikasi Validation Error & Cek Sukses -->
            @error('email')
                <div class="p-3 bg-red-50 text-red-600 rounded-lg text-sm mb-4">
                    {{ $message }}
                </div>
            @enderror

            <div class="space-y-2">
              <label for="email" class="text-sm font-medium leading-none text-gray-700">Email Akses Admin</label>
              <div class="relative">
                ...
                <input
                  ...
                  class="flex h-10 w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:cursor-not-allowed disabled:opacity-50 pl-10"
                  required
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              class="w-full flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white font-medium h-10 px-4 py-2 rounded-md transition-colors"
            >
          ...
          <div class="mt-6 p-4 bg-blue-50 rounded-lg">
            <p class="text-sm text-blue-800">
              <span class="font-semibold">Info:</span> Kode verifikasi akan dikirim ke email admin untuk keperluan login sistem.
            </p>
          </div>
        </div>
